...
class Editor extends import_eventemitter3.EventEmitter {
  constructor({ store, user, shapeUtils, tools, getContainer, initialState }) {
    super();
    this.store = store;
    ...
    const reparentArrow = (arrowId) => {
      ...
      if (higherSiblings.length) {
        ...
        finalIndex = (0, import_reordering.getIndexBetween)(highestSibling.index, higherSiblings[0].index);
      } else {
        finalIndex = (0, import_reordering.getIndexAbove)(highestSibling.index);
      }
  ...
  getArrowsBoundTo(shapeId) {
    return this._arrowBindingsIndex.value[shapeId] || import_state.EMPTY_ARRAY;
  }
  get arrowInfoCache() {
  ...
  get selectionPageBounds() {
    ...
    if (selectedShapeIds.length === 0)
      return null;
    return import_Box2d.Box2d.Common((0, import_utils.compact)(selectedShapeIds.map((id) => this.getShapePageBounds(id))));
  }
  ...
  setCamera(point, animation) {
    ...
    if (animation) {
      ...
      return this._animateToViewport(new import_Box2d.Box2d(-x, -y, width / z, height / z), animation);
    } else {
  ...
  zoomToFit(animation) {
    ...
    if (ids.length <= 0)
      return this;
    const pageBounds = import_Box2d.Box2d.Common((0, import_utils.compact)(ids.map((id) => this.getShapePageBounds(id))));
  ...
  panZoomIntoView(ids, animation) {
    ...
    if (ids.length <= 0)
      return this;
    const selectionBounds = import_Box2d.Box2d.Common((0, import_utils.compact)(ids.map((id) => this.getShapePageBounds(id))));
  ...
  slideCamera(opts = {}) {
    ...
    const moveCamera = (elapsed) => {
      const { x: cx, y: cy, z: cz } = this.camera;
      const movementVec = import_Vec2d.Vec2d.Mul(direction, currentSpeed * elapsed / cz);
  ...
  updateViewportScreenBounds(center = false) {
    ...
    const screenBounds = new import_Box2d.Box2d(
      rect.left || rect.x,
      rect.top || rect.y,
  ...
  get viewportScreenBounds() {
    ...
    return new import_Box2d.Box2d(x, y, w, h);
  }
  ...
  get viewportPageBounds() {
    ...
    const { x: cx, y: cy, z: cz } = this.camera;
    return new import_Box2d.Box2d(-cx, -cy, w / cz, h / cz);
  }
  ...
  startFollowingUser(userId) {
    ...
    const moveTowardsUser = () => {
      ...
      const leaderScreen = import_Box2d.Box2d.From(leaderPresence.screenBounds);
      ...
      const leaderCenter = new import_Vec2d.Vec2d(
      ...
      const targetZoom = (0, import_utils2.clamp)(this.camera.z * ratio, import_constants.MIN_ZOOM, import_constants.MAX_ZOOM);
      ...
      const targetCenter = import_Vec2d.Vec2d.Add(center, import_Vec2d.Vec2d.Mul(displacement, chaseProportion));
      const distance = import_Vec2d.Vec2d.Sub(targetCenter, center).len();
  ...
  /** @internal */
  _renderingBounds = (0, import_state.atom)("rendering viewport", new import_Box2d.Box2d());
  get renderingBoundsExpanded() {
    return this._renderingBoundsExpanded.value;
  }
  /** @internal */
  _renderingBoundsExpanded = (0, import_state.atom)("rendering viewport expanded", new import_Box2d.Box2d());
  /**
  ...
  get pages() {
    return this._pages.value.sort(import_reordering.sortByIndex);
  }
  /**
   * The current page.
   *
  ...
  _createPage = this.history.createCommand(
    ...
    (page) => {
      ...
      if (!index || pages.some((p) => p.index === index)) {
        index = (0, import_reordering.getIndexAbove)(pages[pages.length - 1].index);
      }
  ...
  duplicatePage(page, createId = import_tlschema.PageRecordType.createId()) {
    ...
    this.batch(() => {
      const { pages } = this;
      const index = (0, import_reordering.getIndexBetween)(freshPage.index, pages[pages.indexOf(freshPage) + 1]?.index);
  ...
  getShapeOutlineSegments(shape) {
    return this._shapeOutlineSegmentsCache.get(typeof shape === "string" ? shape : shape.id) ?? import_state.EMPTY_ARRAY;
  }
  ...
  getShapeLocalTransform(shape) {
    ...
    return import_Matrix2d.Matrix2d.Identity().translate(freshShape.x, freshShape.y).rotate(freshShape.rotation);
  }
  get _shapePageTransformCache() {
    return this.store.createComputedCache("pageTransformCache", (shape) => {
      ...
      const parentTransform = this._shapePageTransformCache.get(shape.parentId) ?? import_Matrix2d.Matrix2d.Identity();
      return import_Matrix2d.Matrix2d.Compose(parentTransform, this.getShapeLocalTransform(shape));
    });
  }
  /**
  ...
  getShapeParentTransform(shape) {
    ...
    if (!freshShape || (0, import_tlschema.isPageId)(freshShape.parentId))
      return import_Matrix2d.Matrix2d.Identity();
    return this._shapePageTransformCache.get(freshShape.parentId) ?? import_Matrix2d.Matrix2d.Identity();
  }
  /**
  ...
  getShapePageTransform(shape) {
    ...
    return this._shapePageTransformCache.get(id) ?? import_Matrix2d.Matrix2d.Identity();
  }
  get _shapePageBoundsCache() {
    return this.store.createComputedCache("pageBoundsCache", (shape) => {
      ...
      const result = import_Box2d.Box2d.FromPoints(
        import_Matrix2d.Matrix2d.applyToPoints(pageTransform, this.getShapeGeometry(shape).vertices)
      );
      return result;
    });
  }
  ...
  get _shapeClipPathCache() {
    return this.store.createComputedCache("clipPathCache", (shape) => {
      ...
      const localMask = import_Matrix2d.Matrix2d.applyToPoints(import_Matrix2d.Matrix2d.Inverse(pageTransform), pageMask);
  ...
  get _shapeMaskCache() {
    return this.store.createComputedCache("pageMaskCache", (shape) => {
      ...
      const pageMask = frameAncestors.map(
        ...
        if (intersection) {
          return intersection.map(import_Vec2d.Vec2d.Cast);
        }
        return void 0;
      });
      return pageMask;
  ...
  getShapeMaskedPageBounds(shape) {
    ...
    if (pageMask) {
      const intersection = (0, import_intersect.intersectPolygonPolygon)(pageMask, pageBounds.corners);
      if (!intersection)
        return;
      return import_Box2d.Box2d.FromPoints(intersection);
    }
    return pageBounds;
  }
  /**
  ...
  getPointInParentSpace(shape, point) {
    ...
    if ((0, import_tlschema.isPageId)(freshShape.parentId))
      return import_Vec2d.Vec2d.From(point);
  ...
  get currentPageShapesSorted() {
    const shapes = new Set(this.currentPageShapes.sort(import_reordering.sortByIndex));
    const results = [];
  ...
  reparentShapes(shapes, parentId, insertIndex) {
    const ids = typeof shapes[0] === "string" ? shapes : shapes.map((s) => s.id);
    const changes = [];
    const parentTransform = (0, import_tlschema.isPageId)(parentId) ? import_Matrix2d.Matrix2d.Identity() : this.getShapePageTransform(parentId);
    const parentPageRotation = parentTransform.rotation();
    let indices = [];
    const sibs = (0, import_utils.compact)(this.getSortedChildIdsForParent(parentId).map((id2) => this.getShape(id2)));
    if (insertIndex) {
      const sibWithInsertIndex = sibs.find((s) => s.index === insertIndex);
      if (sibWithInsertIndex) {
        const sibAbove = sibs[sibs.indexOf(sibWithInsertIndex) + 1];
        if (sibAbove) {
          indices = (0, import_reordering.getIndicesBetween)(insertIndex, sibAbove.index, ids.length);
        } else {
          indices = (0, import_reordering.getIndicesAbove)(insertIndex, ids.length);
        }
      } else {
        const sibAbove = sibs.sort(import_reordering.sortByIndex).find((s) => s.index > insertIndex);
        if (sibAbove) {
          indices = (0, import_reordering.getIndicesBetween)(insertIndex, sibAbove.index, ids.length);
        } else {
          indices = (0, import_reordering.getIndicesAbove)(insertIndex, ids.length);
      ...
      indices = sib ? (0, import_reordering.getIndicesAbove)(sib.index, ids.length) : (0, import_reordering.getIndices)(ids.length);
    }
  ...
  getHighestIndexForParent(parent) {
    ...
    return (0, import_reordering.getIndexAbove)(shape.index);
  }
  /**
  ...
  nudgeShapes(shapes, offset, historyOptions) {
    ...
    for (const id of ids) {
      ...
      const localDelta = import_Vec2d.Vec2d.Cast(offset);
    ...
    const shapesToCreate = (0, import_utils.compact)(
      idsToCreate.map((id) => {
        ...
        if (offset && initialIds.has(id)) {
          const parentTransform = this.getShapeParentTransform(shape);
          const vec = new import_Vec2d.Vec2d(offset.x, offset.y).rot(-parentTransform.rotation());
          ox = vec.x;
          oy = vec.y;
        }
        ...
        const index = siblingAbove ? (0, import_reordering.getIndexBetween)(shape.index, siblingAbove.index) : (0, import_reordering.getIndexAbove)(shape.index);
        ...
        if (this.isShapeOfType(shape, "arrow") && this.isShapeOfType(newShape, "arrow")) {
          ...
          if (info?.isValid && infoAfter?.isValid && !(0, import_shared.getIsArrowStraight)(shape)) {
            const mpA = import_Vec2d.Vec2d.Med(info.start.handle, info.end.handle);
            const distA = import_Vec2d.Vec2d.Dist(info.middle, mpA);
            const distB = import_Vec2d.Vec2d.Dist(infoAfter.middle, mpA);
  ...
  flipShapes(shapes, operation) {
    ...
    const scaleOriginPage = import_Box2d.Box2d.Common(
  ...
  stackShapes(shapes, operation, gap) {
    ...
    shapesToStack.forEach((shape, i) => {
      ...
      const localDelta = parent ? import_Vec2d.Vec2d.Rot(delta, -this.getShapePageTransform(parent).decompose().rotation) : delta;
      const translateStartChanges = this.getShapeUtil(shape).onTranslateStart?.(shape);
      changes.push(
        translateStartChanges ? {
          ...translateStartChanges,
  ...
  packShapes(shapes, gap) {
    ...
    const commonBounds = import_Box2d.Box2d.Common((0, import_utils.compact)(Object.values(shapePageBounds)));
    const maxWidth = commonBounds.width;
    shapesToPack.sort((a, b) => shapePageBounds[b.id].height - shapePageBounds[a.id].height);
    const startWidth = Math.max(Math.ceil(Math.sqrt(area / 0.95)), maxWidth);
    const spaces = [new import_Box2d.Box2d(commonBounds.x, commonBounds.y, startWidth, Infinity)];
    let width = 0;
    let height = 0;
    let space;
    let last;
    ...
    const commonAfter = import_Box2d.Box2d.Common(Object.values(nextShapePageBounds));
    const centerDelta = import_Vec2d.Vec2d.Sub(commonBounds.center, commonAfter.center);
    let nextBounds;
    const changes = [];
    for (let i = 0; i < shapesToPack.length; i++) {
      shape = shapesToPack[i];
      ...
      const delta = import_Vec2d.Vec2d.Sub(nextBounds.point, bounds.point).add(centerDelta);
      const parentTransform = this.getShapeParentTransform(shape);
      if (parentTransform)
  ...
  distributeShapes(shapes, operation) {
    ...
    shapesToDistribute.filter((shape) => shape !== first && shape !== last).sort((a, b) => pageBounds[a.id][mid] - pageBounds[b.id][mid]).forEach((shape, i) => {
      ...
      const parent = this.getShapeParent(shape);
      const localDelta = parent ? import_Vec2d.Vec2d.Rot(delta, -this.getShapePageTransform(parent).rotation()) : delta;
      const translateStartChanges = this.getShapeUtil(shape).onTranslateStart?.(shape);
      changes.push(
        translateStartChanges ? {
          ...translateStartChanges,
      ...
      case "vertical": {
        this.batch(() => {
          for (const shape of shapesToStretch) {
            ...
            const pageBounds = shapePageBounds[shape.id];
            const localOffset = new import_Vec2d.Vec2d(0, commonBounds.minY - pageBounds.minY);
            const parentTransform = this.getShapeParentTransform(shape);
            if (parentTransform)
              localOffset.rot(-parentTransform.rotation());
            const { x, y } = import_Vec2d.Vec2d.Add(localOffset, shape);
            this.updateShapes([{ id: shape.id, type: shape.type, x, y }], { squashing: true });
            const scale = new import_Vec2d.Vec2d(1, commonBounds.height / pageBounds.height);
            this.resizeShape(shape.id, scale, {
              initialBounds: bounds,
              scaleOrigin: new import_Vec2d.Vec2d(pageBounds.center.x, commonBounds.minY),
              scaleAxisRotation: 0
            });
          }
        });
      ...
      case "horizontal": {
        this.batch(() => {
          for (const shape of shapesToStretch) {
            ...
            if (pageRotation % import_utils2.PI2)
              continue;
            const localOffset = new import_Vec2d.Vec2d(commonBounds.minX - pageBounds.minX, 0);
            const parentTransform = this.getShapeParentTransform(shape);
            if (parentTransform)
              localOffset.rot(-parentTransform.rotation());
            const { x, y } = import_Vec2d.Vec2d.Add(localOffset, shape);
            this.updateShapes([{ id: shape.id, type: shape.type, x, y }], { squashing: true });
            const scale = new import_Vec2d.Vec2d(commonBounds.width / pageBounds.width, 1);
            this.resizeShape(shape.id, scale, {
              initialBounds: bounds,
              scaleOrigin: new import_Vec2d.Vec2d(commonBounds.minX, pageBounds.center.y),
              scaleAxisRotation: 0
            });
          }
        });
  ...
  resizeShape(shape, scale, options = {}) {
    ...
    if (!Number.isFinite(scale.x))
      scale = new import_Vec2d.Vec2d(1, scale.y);
    if (!Number.isFinite(scale.y))
      scale = new import_Vec2d.Vec2d(scale.x, 1);
    ...
    if (!scaleOrigin)
      return this;
    const pageTransform = options.initialPageTransform ? import_Matrix2d.Matrix2d.Cast(options.initialPageTransform) : this.getShapePageTransform(id);
    if (!pageTransform)
      return this;
    ...
    if (util.isAspectRatioLocked(initialShape)) {
      if (Math.abs(scale.x) > Math.abs(scale.y)) {
        scale = new import_Vec2d.Vec2d(scale.x, Math.sign(scale.y) * Math.abs(scale.x));
      } else {
        scale = new import_Vec2d.Vec2d(Math.sign(scale.x) * Math.abs(scale.y), scale.y);
      }
    }
    if (util.onResize && util.canResize(initialShape)) {
      const newPagePoint = this._scalePagePoint(
        import_Matrix2d.Matrix2d.applyToPoint(pageTransform, new import_Vec2d.Vec2d(0, 0)),
        scaleOrigin,
        scale,
        scaleAxisRotation
      );
      const newLocalPoint = this.getPointInParentSpace(initialShape.id, newPagePoint);
      const myScale = new import_Vec2d.Vec2d(scale.x, scale.y);
      ...
      const initialPagePoint = import_Matrix2d.Matrix2d.applyToPoint(pageTransform, new import_Vec2d.Vec2d());
      ...
      const initialPageCenter = import_Matrix2d.Matrix2d.applyToPoint(pageTransform, initialBounds.center);
      ...
      const delta = import_Vec2d.Vec2d.Sub(newPageCenterInParentSpace, initialPageCenterInParentSpace);
      this.updateShapes(
        [
          {
            id,
  ...
  _scalePagePoint(point, scaleOrigin, scale, scaleAxisRotation) {
    const relativePoint = import_Vec2d.Vec2d.RotWith(point, scaleOrigin, -scaleAxisRotation).sub(scaleOrigin);
    const newRelativePagePoint = import_Vec2d.Vec2d.MulV(relativePoint, scale);
    const destination = import_Vec2d.Vec2d.Add(newRelativePagePoint, scaleOrigin).rotWith(
      scaleOrigin,
      scaleAxisRotation
    );
    return destination;
  ...
  _resizeUnalignedShape(id, scale, options) {
    ...
    const shapeScale = new import_Vec2d.Vec2d(scale.x, scale.y);
    if (Math.abs(scale.x) > Math.abs(scale.y)) {
    ...
    if (Math.sign(scale.x) * Math.sign(scale.y) < 0) {
      let { rotation } = import_Matrix2d.Matrix2d.Decompose(options.initialPageTransform);
      rotation -= 2 * rotation;
      this.updateShapes([{ id, type, rotation }], { squashing: true });
    }
    const preScaleShapePageCenter = import_Matrix2d.Matrix2d.applyToPoint(
    ...
    if (!currentPageCenter || !shapePageTransformOrigin)
      return this;
    const pageDelta = import_Vec2d.Vec2d.Sub(postScaleShapePageCenter, currentPageCenter);
    const postScaleShapePagePoint = import_Vec2d.Vec2d.Add(shapePageTransformOrigin, pageDelta);
      ...
      do: ({ partials }) => {
        ...
        partials = partials.map((partial) => {
          if (!partial.parentId || !(this.store.has(partial.parentId) || partials.some((p) => p.id === partial.parentId))) {
            partial = { ...partial };
            const parentId = sortedShapesOnCurrentPage.findLast(
              (parent) => (
                // parent.type === 'frame'
        ...
        for (const partial of partials) {
          ...
          if (!index) {
            ...
            parentIndices.set(parentId, (0, import_reordering.getIndexAbove)(index));
          }
  ...
  groupShapes(shapes, groupId = (0, import_tlschema.createShapeId)()) {
    ...
    const sortedShapeIds = shapesToGroup.sort(import_reordering.sortByIndex).map((s) => s.id);
    const pageBounds = import_Box2d.Box2d.Common((0, import_utils.compact)(shapesToGroup.map((id) => this.getShapePageBounds(id))));
    ...
    const shapesWithRootParent = shapesToGroup.filter((shape) => shape.parentId === parentId).sort(import_reordering.sortByIndex);
  ...
  _updateShapes = this.history.createCommand(
    ...
    (_partials, historyOptions) => {
      ...
      const snapshots = Object.fromEntries(
        (0, import_utils.compact)(partials.map(({ id }) => this.getShape(id))).map((shape) => {
   ...
   * @internal
   */
  _selectionSharedStyles = (0, import_state.computed)(
  ...
  getContentFromCurrentPage(shapes) {
    ...
    let shapesForContent = (0, import_utils.dedupe)(
      ids.map((id) => this.getShape(id)).sort(import_reordering.sortByIndex).flatMap((shape) => {
  ...
  putContentOntoCurrentPage(content, options = {}) {
    ...
    const newShapes = shapes.map((shape) => {
      ...
      if (idMap.has(newShape.parentId)) {
        ...
        index = (0, import_reordering.getIndexAbove)(index);
      }
...
reason:${result.reason}`
    ...
    this.batch(() => {
      ...
      const bounds = import_Box2d.Box2d.Common(newCreatedShapes.map((s) => this.getShapePageBounds(s)));
      if (point === void 0) {
        if (!(0, import_tlschema.isPageId)(pasteParentId)) {
          const shape = this.getShape(pasteParentId);
          point = import_Matrix2d.Matrix2d.applyToPoint(
            this.getShapePageTransform(shape),
            this.getShapeGeometry(shape).bounds.center
          );
        } else {
          const { viewportPageBounds } = this;
          if (preservePosition || viewportPageBounds.includes(import_Box2d.Box2d.From(bounds))) {
            point = bounds.center;
          } else {
      ...
      const pageCenter = import_Box2d.Box2d.Common(
        (0, import_utils.compact)(rootShapes.map(({ id }) => this.getShapePageBounds(id)))
      ).center;
      const offset = import_Vec2d.Vec2d.Sub(point, pageCenter);
      this.updateShapes(
        rootShapes.map(({ id }) => {
          const s = this.getShape(id);
          const localRotation = this.getShapeParentTransform(id).decompose().rotation;
          const localDelta = import_Vec2d.Vec2d.Rot(offset, -localRotation);
  ...
  async getSvg(shapes, opts = {}) {
    ...
    const theme = (0, import_tlschema.getDefaultColorTheme)({ isDarkMode: this.user.isDarkMode });
  ...
  inputs = {
    ...
    originPagePoint: new import_Vec2d.Vec2d(),
    /** The most recent pointer down's position in screen space. */
    originScreenPoint: new import_Vec2d.Vec2d(),
    /** The previous pointer position in the current page space. */
    previousPagePoint: new import_Vec2d.Vec2d(),
    /** The previous pointer position in screen space. */
    previousScreenPoint: new import_Vec2d.Vec2d(),
    /** The most recent pointer position in the current page space. */
    currentPagePoint: new import_Vec2d.Vec2d(),
    /** The most recent pointer position in screen space. */
    currentScreenPoint: new import_Vec2d.Vec2d(),
    ...
    pointerVelocity: new import_Vec2d.Vec2d()
  };
  /**
      ...
      switch (type) {
        ...
        case "wheel": {
          ...
          if (this.isMenuOpen) {
            ...
            this.pan(info.delta);
            if (!inputs.isDragging && inputs.isPointing && originPagePoint.dist(currentPagePoint) > (this.instanceState.isCoarsePointer ? import_constants.COARSE_DRAG_DISTANCE : import_constants.DRAG_DISTANCE) / this.zoomLevel) {
          ...
          switch (info.name) {
            ...
            case "pointer_move": {
              ...
              if (this.inputs.isPanning && this.inputs.isPointing) {
                ...
                this.pan(import_Vec2d.Vec2d.Sub(currentScreenPoint2, previousScreenPoint));
                return;
              }
              if (!inputs.isDragging && inputs.isPointing && originPagePoint.dist(currentPagePoint) > (this.instanceState.isCoarsePointer ? import_constants.COARSE_DRAG_DISTANCE : import_constants.DRAG_DISTANCE) / this.zoomLevel) {
                inputs.isDragging = true;
