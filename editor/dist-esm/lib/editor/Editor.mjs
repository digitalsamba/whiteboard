...
import {
  ...
  getIndicesBetween,
  sortByIndex
} from "../utils/reordering/reordering.mjs";
...
import { SideEffectManager } from "./managers/SideEffectManager.mjs";
...
import { getCurvedArrowInfo } from "./shapes/shared/arrow/curved-arrow.mjs";
import { getArrowTerminalsInArrowSpace, getIsArrowStraight } from "./shapes/shared/arrow/shared.mjs";
import { getStraightArrowInfo } from "./shapes/shared/arrow/straight-arrow.mjs";
...
class Editor extends EventEmitter {
  ...
  /** @internal */
  _renderingBounds = atom("rendering viewport", new Box2d());
  get renderingBoundsExpanded() {
    return this._renderingBoundsExpanded.value;
  }
  /** @internal */
  _renderingBoundsExpanded = atom("rendering viewport expanded", new Box2d());
  /**
  ...
  reparentShapes(shapes, parentId, insertIndex) {
    ...
    let indices = [];
    const sibs = compact(this.getSortedChildIdsForParent(parentId).map((id2) => this.getShape(id2)));
    if (insertIndex) {
  ...
  duplicateShapes(shapes, offset) {
    ...
    const shapesToCreate = compact(
      idsToCreate.map((id) => {
        ...
        const index = siblingAbove ? getIndexBetween(shape.index, siblingAbove.index) : getIndexAbove(shape.index);
   ...
   * @internal
   */
  _selectionSharedStyles = computed(
  ...
  getContentFromCurrentPage(shapes) {
    ...
    let shapesForContent = dedupe(
      ids.map((id) => this.getShape(id)).sort(sortByIndex).flatMap((shape) => {
    ...
    this.store.put([
      {
        ...
        lastActivityTimestamp: (
          ...
          (info.type === "pointer" && info.pointerId === INTERNAL_POINTER_IDS.CAMERA_MOVE ? this.store.get(TLPOINTER_ID)?.lastActivityTimestamp ?? Date.now() : Date.now())
        ),
        meta: {}
      }
    ]);
