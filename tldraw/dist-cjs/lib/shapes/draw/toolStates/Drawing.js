...
class Drawing extends import_editor.StateNode {
  ...
  onPointerMove = () => {
    ...
    if (this.canDraw) {
      if (inputs.isPen) {
        if (import_editor.Vec2d.Dist(inputs.currentPagePoint, this.lastRecordedPoint) >= 1 / this.editor.zoomLevel) {
  ...
  getIsClosed(segments, size) {
    ...
    return firstPoint !== lastPoint && this.currentLineLength > strokeWidth * 4 && import_editor.Vec2d.Dist(firstPoint, lastPoint) < strokeWidth * 2;
  }
  startShape() {
    const {
    ...
    if (this.initialShape) {
      ...
      if (shape && this.segmentMode === "straight") {
        ...
        const prevPointPageSpace = import_editor.Matrix2d.applyToPoint(
  ...
  updateShapes() {
    ...
    switch (this.segmentMode) {
      case "starting_straight": {
        ...
        const hasMovedFarEnough = import_editor.Vec2d.Dist(pagePointWhereNextSegmentChanged, inputs.currentPagePoint) > import_editor.DRAG_DISTANCE;
        if (hasMovedFarEnough) {
          ...
          if (prevSegment.type === "straight") {
            this.currentLineLength += import_editor.Vec2d.Dist(prevLastPoint, newLastPoint);
            newSegment = {
              type: "straight",
              points: [{ ...prevLastPoint }, newLastPoint]
            };
            const transform = this.editor.getShapePageTransform(shape);
            this.pagePointWhereCurrentSegmentChanged = import_editor.Matrix2d.applyToPoint(
              transform,
              prevLastPoint
            );
          } else {
      ...
      case "starting_free": {
        ...
        if (hasMovedFarEnough) {
          ...
          const newFreeSegment = {
            type: "free",
            points: [
              ...import_editor.Vec2d.PointsBetween(prevPoint, newPoint, 6).map((p) => p.toFixed().toJson())
            ]
          };
      ...
      case "straight": {
        ...
        if (shouldSnap) {
          if (newSegments.length > 2) {
            ...
            for (let i = 0, n = segments.length - 2; i < n; i++) {
              ...
              if (!(first && lastPoint))
                continue;
              const nearestPointOnSegment = import_editor.Vec2d.NearestPointOnLineSegment(
                first,
                lastPoint,
                newPoint2
              );
              const distance = import_editor.Vec2d.Dist(nearestPointOnSegment, newPoint2);
        ...
        if (didSnap && snapSegment) {
          ...
          const A = import_editor.Matrix2d.applyToPoint(transform, first);
          const B = import_editor.Matrix2d.applyToPoint(transform, lastPoint);
          const snappedPoint = import_editor.Matrix2d.applyToPoint(transform, newPoint2);
          ...
          if (shouldSnapToAngle) {
            const currentAngle = import_editor.Vec2d.Angle(pagePointWhereCurrentSegmentChanged, currentPagePoint);
            const snappedAngle = (0, import_editor.snapAngle)(currentAngle, 24);
            const angleDiff = snappedAngle - currentAngle;
            pagePoint = import_editor.Vec2d.RotWith(
              currentPagePoint,
              pagePointWhereCurrentSegmentChanged,
              angleDiff
            );
        ...
        this.currentLineLength += import_editor.Vec2d.Dist(newSegment.points[0], newPoint2);
      ...
      case "free": {
        ...
        if (newPoints.length && this.mergeNextPoint) {
          ...
          this.currentLineLength += import_editor.Vec2d.Dist(newPoints[newPoints.length - 1], newPoint);
          ...
          this.editor.createShapes([
            {
              id: newShapeId,
              type: this.shapeType,
              x: (0, import_editor.toFixed)(currentPagePoint.x),
              y: (0, import_editor.toFixed)(currentPagePoint.y),
              props: {
                isPen: this.isPen,
                segments: [
                  {
  ...
  getLineLength(segments) {
    ...
    for (const segment of segments) {
      for (let i = 0; i < segment.points.length - 1; i++) {
        ...
        length += import_editor.Vec2d.Sub(B, A).len2();
      }
    }
