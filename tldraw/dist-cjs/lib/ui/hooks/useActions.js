...
var import_usePrint = require("./usePrint");
...
function ActionsProvider({ overrides, children }) {
  ...
  const { clearToasts } = (0, import_useToastsProvider.useToasts)();
  ...
  const actions = React.useMemo(() => {
    ...
    const actions2 = makeActions([
      ...
      },
      {
        ...
        kbd: "z",
        onSelect(source) {
          if (editor.root.current.value?.id === "zoom")
            return;
          ...
          editor.batch(() => {
            ...
            for (const shape of shapes) {
              if (!shape || !editor.isShapeOfType(shape, "embed") || !shape.props.url)
                continue;
              const newPos = new import_editor.Vec2d(shape.x, shape.y);
              newPos.rot(-shape.rotation);
              newPos.add(new import_editor.Vec2d(shape.props.w / 2 - 300 / 2, shape.props.h / 2 - 320 / 2));
              newPos.rot(shape.rotation);
              const partial = {
        ...
        onSelect(source) {
          ...
          editor.batch(() => {
            ...
            for (const shape of shapes) {
              ...
              const newPos = new import_editor.Vec2d(shape.x, shape.y);
              newPos.rot(-shape.rotation);
              newPos.add(new import_editor.Vec2d(shape.props.w / 2 - width / 2, shape.props.h / 2 - height / 2));
              newPos.rot(shape.rotation);
              const shapeToCreate = {
                id: (0, import_editor.createShapeId)(),
                type: "embed",
      ...
      },
      {
        ...
        onSelect(source) {
          ...
          const ids = editor.selectedShapeIds;
          const commonBounds = import_editor.Box2d.Common((0, import_editor.compact)(ids.map((id) => editor.getShapePageBounds(id))));
      ...
      },
      {
        ...
        onSelect(source) {
          ...
          const offset = editor.selectionRotation % (import_editor.TAU / 2);
          const dontUseOffset = (0, import_editor.approximately)(offset, 0) || (0, import_editor.approximately)(offset, import_editor.TAU / 2);
          editor.rotateShapesBy(editor.selectedShapeIds, import_editor.TAU / 2 - (dontUseOffset ? 0 : offset));
        }
      },
      {
        id: "rotate-ccw",
        ...
        onSelect(source) {
          ...
          editor.rotateShapesBy(editor.selectedShapeIds, offsetCloseToZero ? -(import_editor.TAU / 2) : -offset);
        }
      },
      {
        id: "zoom-in",
